...
data = readRDS('../data/pathway_scores.rds')
mat = data[['res']][['GO_BP']][['scores_top_20']]
mat2 = data[['res']][['apoe_associated']][['scores_sig']]
mat3 = data[['res']][['lipid_associated']][['scores']]

# import the renamed-pathway names
print('importing pathway data')
path_names_go = as.data.frame(read_excel('../table_outputs/all_path_names_go_renaming.xlsx', sheet = 'all_path_names_go_renaming'))
rownames(path_names_go) = path_names_go$name
path_names_apoe = as.data.frame(read_excel('../table_outputs/all_path_names_go_renaming.xlsx', sheet = 'apoe_terms'))
rownames(path_names_apoe) = path_names_apoe[,'full name']
path_names_lipid = as.data.frame(read_excel('../table_outputs/all_path_names_go_renaming.xlsx', sheet = 'lipid_terms'))
rownames(path_names_lipid) = path_names_lipid[,'full name']
names = read.csv('../data/sele_pathways.csv')[,1]
...
mat = get_matrix(mat3$all, names)
df = mat[unname(rowSums(abs(mat)>1.3)>0),]

temp = as.data.frame((abs(df)>1.3) * sign(df))
temp_order = rownames(temp[order((temp$Ex),(temp$In), (temp$Ast), (temp$Mic), (temp$Oli), (temp$Opc), decreasing = T),])

df = df[temp_order,c('Ex','In','Ast', 'Mic', 'Oli', 'Opc')]
cell_fun = function(j, i, x, y, width, height, fill) {
